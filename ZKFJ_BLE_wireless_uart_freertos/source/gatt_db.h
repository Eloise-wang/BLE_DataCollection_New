...
PRIMARY_SERVICE(service_gap, gBleSig_GenericAccessProfile_d)
    CHARACTERISTIC(char_device_name, gBleSig_GapDeviceName_d, (gGattCharPropRead_c) )
            VALUE(value_device_name, gBleSig_GapDeviceName_d, (gPermissionFlagReadable_c), 8, "ZZL_0001")
...
PRIMARY_SERVICE_UUID128(service_wireless_uart, uuid_service_wireless_uart)
    ...
    CHARACTERISTIC_UUID128(char_uart_notify, uuid_uart_notify, (gGattCharPropNotify_c))
        VALUE_UUID128_VARLEN(value_uart_notify, uuid_uart_notify, (gPermissionFlagReadable_c), gAttMaxWriteDataSize_d(gAttMaxMtu_c), 1, 0x00)
        CCCD(cccd_uart_notify)
